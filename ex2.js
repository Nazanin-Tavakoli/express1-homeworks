...
app.get('/:category/:subcategory/:id', (req, res) => {
  ...
  const categoryData = storeData.find(cat => cat.category === category);

  if (!categoryData) {
    return res.status(404).send('Did not find the category');
  }

  const subcategoryData = categoryData.subcategories.find(subcat => subcat.subcategory === subcategory);

  if (!subcategoryData) {
    return res.status(404).send('Did not find the subcategory');
  }

  const productData = subcategoryData.products.find(pd => pd.id === parseInt(id));
